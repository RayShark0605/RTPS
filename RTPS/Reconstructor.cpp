...
int CReconstructor::Reconstruct(string ImagePath, CMapperOptions* MapperOptions, DatabaseCache& DbCache, QSqlDatabase& db)
{
	...
	while (IsThisRegSuccess) //退出条件: 上一次注册(IsPreRegSuccess)失败了, 这一次注册(IsThisRegSuccess)也失败了
	{
		if (!IsContinue)
		{
			ModelManager->UnLock();
		...
		for (image_t NextImageID : NextImages) //遍历待注册的影像, 直到找到一个注册成功的影像
		{
			if (!IsContinue)
			{
				ModelManager->UnLock();
			...
			else
			{
				...
				// 如果尝试了50张影像还没注册成功, 而且当前模型的注册影像数还太少, 那么不继续尝试了
		...
		//到这里, 如果IsPreRegSuccess和IsThisRegSuccess都为false, 那么之后会退出while循环
		if (!IsThisRegSuccess && IsPreRegSuccess) //如果这一次尝试失败了, 但是上一次是成功的, 那么做一次全局精化之后, 再给一次机会
		{
