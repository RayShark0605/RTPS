...
CTriangulator::CTriangulator(const CorrespondenceGraph* correspondence_graph, CModel* reconstruction)
{
	correspondence_graph_ = correspondence_graph;
	reconstruction_ = reconstruction;
}
size_t CTriangulator::TriangulateImage(const Options& options, const image_t image_id)
{
    CHECK(options.Check());
    ...
    {
        const size_t num_triangulated = Find(options, image_id, point2D_idx, static_cast<size_t>(options.max_transitivity), &corrs_data);
        if (corrs_data.empty())
        {
            continue;
        }
...
size_t CTriangulator::CompleteImage(const Options& options, const image_t image_id)
{
    CHECK(options.Check());
    ...
    for (point2D_t point2D_idx = 0; point2D_idx < image.NumPoints2D();
        ...
        const point3D_t point3D_id =
            reconstruction_->ModelAddPoint3D(xyz, track);
        modified_point3D_ids_.insert(point3D_id);
    }

    return num_tris;
}
size_t CTriangulator::CompleteTracks(const Options& options, const unordered_set<point3D_t>& point3D_ids)
{
    CHECK(options.Check());
...
size_t CTriangulator::CompleteAllTracks(const Options& options) 
{
    ...
    for (const point3D_t point3D_id : reconstruction_->GetModelAllPoint3Ds()) 
    {
        num_completed += Complete(options, point3D_id);
    }
...
size_t CTriangulator::MergeTracks(const Options& options, const unordered_set<point3D_t>& point3D_ids) 
{
    CHECK(options.Check());
...
size_t CTriangulator::Retriangulate(const Options& options) 
{
    CHECK(options.Check());
    ...
    for (const auto& image_pair : reconstruction_->GetModelImagePairs())
    {
        ...
        const Image& image1 = reconstruction_->GetReferImage(image_id1);
        if (!image1.IsRegistered()) {
            continue;
        }

        const Image& image2 = reconstruction_->GetReferImage(image_id2);
        ...
        const Camera& camera1 = reconstruction_->GetReferCamera(image1.CameraId());
        const Camera& camera2 = reconstruction_->GetReferCamera(image2.CameraId());
...
void CTriangulator::AddModifiedPoint3D(const point3D_t point3D_id) 
{
    modified_point3D_ids_.insert(point3D_id);
}
const unordered_set<point3D_t>& CTriangulator::GetModifiedPoints3D() 
{
    // First remove any missing 3D points from the set.
    for (auto it = modified_point3D_ids_.begin();
        it != modified_point3D_ids_.end();) {
        if (reconstruction_->IsModelExistPoint3D(*it)) 
        {
            ++it;
        }
        else 
...
size_t CTriangulator::Find(const Options& options, const image_t image_id, const point2D_t point2D_idx, const size_t transitivity, std::vector<CorrData>* corrs_data)
{
    const std::vector<CorrespondenceGraph::Correspondence>* found_corrs_ptr = nullptr;
    if (transitivity == 1) 
    {
        found_corrs_ptr = &correspondence_graph_->FindCorrespondences(image_id, point2D_idx);
    }
    else 
    {
        correspondence_graph_->FindTransitiveCorrespondences(image_id, point2D_idx, transitivity, &found_corrs_);
    ...
    {
        const Image& corr_image = reconstruction_->GetReferImage(corr.image_id);
        if (!corr_image.IsRegistered()) {
            continue;
        }

        const Camera& corr_camera = reconstruction_->GetReferCamera(corr_image.CameraId());
...
size_t CTriangulator::Create(const Options& options, const std::vector<CorrData>& corrs_data) {
    ...
    for (size_t i = 0; i < inlier_mask.size(); ++i) 
    {
        ...
        {
            const CorrData& corr_data = create_corrs_data[i];
            if (!reconstruction_->GetModelImage(corr_data.image_id).Point2D(corr_data.point2D_idx).HasPoint3D())
            {
                track.AddElement(corr_data.image_id, corr_data.point2D_idx);
            }
        }
    ...
    const point3D_t point3D_id = reconstruction_->ModelAddPoint3D(xyz, track);
    modified_point3D_ids_.insert(point3D_id);
...
size_t CTriangulator::Continue(const Options& options, const CorrData& ref_corr_data,const std::vector<CorrData>& corrs_data) {
    ...
    for (size_t idx = 0; idx < corrs_data.size(); ++idx) {
        ...
        const Point3D& point3D = reconstruction_->GetModelPoint3D(corr_data.point2D->Point3DId());
    ...
    if (best_angle_error <= max_angle_error &&
        ...
        TrackElement track_el(ref_corr_data.image_id, ref_corr_data.point2D_idx);
    ...
    for (const auto& track_el : point3D.Track().Elements()) {
        ...
        for (const auto corr : corrs) {
            ...
            if (merge_success) {
                ...
                const point3D_t merged_point3D_id = reconstruction_->MergePoint3Ds(
    ...
    for (int transitivity = 0; transitivity < max_transitivity; ++transitivity) {
        ...
        for (const TrackElement queue_elem : prev_queue) {
            ...
            for (const auto corr : corrs) {
                ...
                TrackElement track_el(corr.image_id, corr.point2D_idx);
...
bool CTriangulator::HasCameraBogusParams(const Options& options,const Camera& camera) {
