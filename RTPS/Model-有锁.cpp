...
Camera CModel::GetModelCamera(size_t CameraID)
{
	...
	{
		Cameras_Mutex.unlock();
		throw "This camera does not exists!";
	}
...
Image CModel::GetModelImage(size_t ImageID)
{
	...
	{
		Images_Mutex.unlock();
		throw "This image does not exists!";
	}
...
Point3D CModel::GetModelPoint3D(size_t Point3DID)
{
	...
	{
		Points3D_Mutex.unlock();
		throw "This point3D does not exists!";
	}
...
{
	Points3D_Mutex.lock();
	auto it1 = Points3D.find(PointID1), it2 = Points3D.find(PointID1);
...
void CModel::AddObservation(size_t Point3DID, TrackElement& element)
{
	...
	if (it == Images.end() || it2 == Points3D.end())
	{
		Images_Mutex.unlock();
		Points3D_Mutex.unlock();
		throw "Corresponding image or 3D point does not exist!";
	}
...
void CModel::DeleteObservation(size_t ImageID, size_t Point2DID)
{
	...
	if (it2 == Points3D.end())
	{
		Images_Mutex.unlock();
		Points3D_Mutex.unlock();
		throw "The corresponding 3D point does not exist!";
	}
...
CModel::CImagePairStatus CModel::GetModelImagePairStatus(size_t ImageID1, size_t ImageID2)
{
	...
	{
		ImagePairs_Mutex.unlock();
		throw "This image pair does not exists!";
	}
...
void CModel::UpdateImageID()
{
	...
	for (auto& pair : Images)
	{
		...
		size_t DbImageID = GetImageID(ImageName);
		CImage DbImage;
		GetImage(DbImageID, DbImage);
...
CModel CModelManager::Get(size_t index)
{
	...
	{
		Models_Mutex.unlock();
		throw "The index of the model is out of range!";
	}
